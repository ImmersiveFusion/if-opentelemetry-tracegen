...
- **Where it runs:** the **Immersive Fusion cloud** — seven grid workloads on Kubernetes, deployed declaratively via GitOps (Argo CD).
- **What for:** **seven public demo grids** — each one a separate TraceGen deployment streaming live OpenTelemetry traces into [Immersive APM](https://immersivefusion.com)'s 3D player, where you can open one and watch *real data moving* instead of a canned recording. Each grid is tuned to tell one story:
