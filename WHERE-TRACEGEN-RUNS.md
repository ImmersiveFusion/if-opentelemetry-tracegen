...
- **What for:** **seven always-on public demo grids** — each one a separate TraceGen deployment streaming live OpenTelemetry traces into [Immersive APM](https://immersivefusion.com)'s 3D player, so anyone can open it and watch *real data moving* instead of a canned recording. Each grid is tuned to tell one story:
